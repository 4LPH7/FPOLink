...
class ForecastingService:
    ...
    def train_or_update_model(
        ...
        if len(df) < 30:
            logger.info(f"Insufficient data ({len(df)} records) for LightGBM; fallback baseline will be used.")
            return None
    ...
    def generate_forecast(
        ...
        for step in range(1, horizon_days + 1):
            ...
            if models is not None:
                ...
                confidence = float(np.clip(1.0 - (upper_bound - lower_bound) / max(predicted_price, 1.0), 0.5, 0.95))
                model_type = "lightgbm_quantile"
                ...
                volatility = float(recent_window.std()) if len(recent_window) > 1 else predicted_price * 0.05
                ...
                lower_bound = round(max(0.0, predicted_price - 1.645 * volatility * spread_expansion), 2)
